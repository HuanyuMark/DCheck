...
public class BuilderAutoDetectIntrospector extends AnnotationIntrospector {
    ...
    @Getter
    @Setter
    private boolean useOtherBuilderClass;
    ...
    protected <A extends Annotation> A _findAnnotation(Annotated ann,
        ...
        try {
            builderMethod = builderClass.getDeclaredMethod(builderConfig.buildMethodName);
        ...
        var builderType = Arrays.stream(innerTypes).filter(candidateBuilderType::isAssignableFrom)
                .findFirst()
                .orElseGet(() -> {
                    if (useOtherBuilderClass && !Modifier.isAbstract(candidateBuilderType.getModifiers()) && !candidateBuilderType.isInterface()) {
    ...
    public Class<?> findPOJOBuilder(AnnotatedClass ac) {
        ...
        Method builderMethod;
        if ((builderMethod = ReflectionUtils.findMethod(builderClass, builderConfig.buildMethodName)) == null) {
            return null;
        }
