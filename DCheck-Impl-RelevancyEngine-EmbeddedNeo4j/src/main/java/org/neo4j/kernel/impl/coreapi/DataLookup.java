...
import static org.neo4j.internal.helpers.collection.Iterators.*;
...
public abstract class DataLookup {
    ...
    public ResourceIterator<Relationship> allRelationshipsWithType(final RelationshipType type) {
    ...
    public ResourceIterator<Relationship> allRelationshipsByTypeWithoutIndex(int typeId) {
    ...
    public ResourceIterator<Node> allNodesWithLabel(Label myLabel) {
        int labelId = tokenRead().nodeLabel(myLabel.name());
    ...
    public ResourceIterator<Node> allNodesByLabelWithoutIndex(int labelId) {
    ...
    public IndexDescriptor findUsableMatchingIndex(
     ...
     */
    public IndexDescriptor findUsableMatchingIndex(SchemaDescriptor schemaDescriptor, IndexQuery... query) {
        return firstOrDefault(getMatchingOnlineIndexes(schemaDescriptor, query), IndexDescriptor.NO_INDEX);
    }

    public Iterator<IndexDescriptor> getMatchingOnlineIndexes(SchemaDescriptor schemaDescriptor, IndexQuery... query) {
    ...
    public ResourceIterator<Node> nodesByLabelAndPropertyWithFiltering(
    ...
    public ResourceIterator<Node> getNodesByLabelAndPropertyWithoutPropertyIndex(
    ...
    public TrackedCursorIterator<FilteringNodeCursorWrapper, Node> getNodesByLabelAndPropertyViaAllNodesScan(
            int labelId, PropertyIndexQuery[] queries) {
        var nodeCursor = cursors().allocateNodeCursor(cursorContext(), memoryTracker());
        ...
        var propertyFilteredCursor = new FilteringNodeCursorWrapper(
                labelFilteredCursor, nodeMatchProperties(queries, propertyCursor), Collections.singletonList(propertyCursor));
    ...
    public ResourceIterator<Node> nodesByLabelAndProperty(
    ...
    public ResourceIterator<Node> nodesByLabelAndProperties(
     ...
     * Find an ONLINE index that matches the schema.
     */
    public IndexDescriptor findUsableMatchingCompositeIndex(
    ...
    public ResourceIterator<Relationship> relationshipsByTypeAndPropertyWithFiltering(
    ...
    public ResourceIterator<Relationship> getRelationshipsByTypeAndPropertyWithoutPropertyIndex(
    ...
    public ResourceIterator<Relationship> getRelationshipsByTypeAndPropertyViaAllRelsScan(
            int typeId, PropertyIndexQuery[] queries) {
        var relationshipScanCursor = cursors().allocateRelationshipScanCursor(cursorContext(), memoryTracker());
        var typeFiltered =
        ...
        var propertyFilteredCursor = new FilteringRelationshipScanCursorWrapper(
                typeFiltered, relationshipMatchProperties(queries, propertyCursor), Collections.singletonList(propertyCursor));
    ...
    public ResourceIterator<Relationship> relationshipsByTypeAndProperty(
    ...
    public ResourceIterator<Relationship> relationshipsByTypeAndProperties(
    ...
    public <T> Iterable<T> allInUse(TokenAccess<T> tokens) {
        return () -> tokens.inUse(dataRead(), schemaRead(), tokenRead());
    }

    public <T> Iterable<T> all(TokenAccess<T> tokens) {
     ...
     */
    public static boolean indexIsOnline(SchemaRead schemaRead, IndexDescriptor index) {
    ...
    public static boolean indexSupportQuery(IndexDescriptor index, IndexQuery[] query) {
        return stream(query).allMatch(q -> index.getCapability().isQuerySupported(q.type(), q.valueCategory()));
    }

    public static void checkPropertyKey(String key) {
        checkArgument(key != null, "Property key can not be null");
    }

    public static void checkLabel(Label label) {
        checkArgument(label != null, "Label can not be null");
    }

    public static boolean invalidTokens(int... tokens) {
        return stream(tokens).anyMatch(token -> token == TokenConstants.NO_TOKEN);
    }

    public static PropertyIndexQuery getIndexQuery(String value, StringSearchMode searchMode, int propertyId) {
        switch (searchMode) {
            case EXACT:
                return PropertyIndexQuery.exact(propertyId, utf8Value(value.getBytes(UTF_8)));
            case PREFIX:
                return PropertyIndexQuery.stringPrefix(propertyId, utf8Value(value.getBytes(UTF_8)));
            case SUFFIX:
                return PropertyIndexQuery.stringSuffix(propertyId, utf8Value(value.getBytes(UTF_8)));
            case CONTAINS:
                return PropertyIndexQuery.stringContains(propertyId, utf8Value(value.getBytes(UTF_8)));
        }
        throw new IllegalArgumentException("Unknown search mode: " + searchMode);
    }

    public static boolean isInvalidQuery(int tokenId, PropertyIndexQuery[] queries) {
    ...
    public static PropertyIndexQuery.ExactPredicate[] convertToQueries(
    ...
    public static int[] getPropertyIds(PropertyIndexQuery[] queries) {
    ...
    public static void assertNoDuplicates(int[] propertyIds, TokenRead tokenRead) {
    ...
    public static boolean hasSamePropertyIds(int[] original, int[] workingCopy, int[] propertyIds) {
    ...
    public static PropertyIndexQuery[] getReorderedIndexQueries(int[] indexPropertyIds, PropertyIndexQuery[] queries) {
    ...
    public static void checkRelationshipType(RelationshipType type) {
        checkArgument(type != null, "Relationship type can not be null");
    }

    public abstract TokenRead tokenRead();

    public abstract SchemaRead schemaRead();

    public abstract Read dataRead();

    public abstract ResourceMonitor resourceMonitor();

    public abstract Node newNodeEntity(long nodeId);

    public abstract Relationship newRelationshipEntity(long relationshipId);

    public abstract Relationship newRelationshipEntity(long id, long startNodeId, int typeId, long endNodeId);

    public abstract CursorFactory cursors();

    public abstract CursorContext cursorContext();

    public abstract MemoryTracker memoryTracker();

    public abstract QueryContext queryContext();

    public abstract ElementIdMapper elementIdMapper();
    ...
    public static class FilteringCursor<CURSOR extends Cursor & ValueIndexCursor> implements Cursor {
        ...
        public final CURSOR originalCursor;
        public final PropertyIndexQuery filteringQuery;
